using CaeriusNet.Core.Exceptions;
...
namespace CaeriusNet.Core.Commands.Reads;
...
public static class SimpleReadSqlAsyncCommands
{
	...
	public static async Task<ReadOnlyCollection<TResultSet>> QueryAsReadOnlyCollectionAsync<TResultSet>(
		...
		where TResultSet : class, ISpMapper<TResultSet>
	{
		if (CacheProvider.TryRetrieveFromCache(spParameters, out ReadOnlyCollection<TResultSet>? cachedResult) &&
		    cachedResult != null)
			return cachedResult;
		...
		try{
			...
			CacheProvider.StoreInCache(spParameters, result);
	...
	public static async Task<IEnumerable<TResultSet>> QueryAsIEnumerableAsync<TResultSet>(
		...
		where TResultSet : class, ISpMapper<TResultSet>
	{
		if (CacheProvider.TryRetrieveFromCache(spParameters, out IEnumerable<TResultSet>? cachedResult) &&
		    cachedResult != null)
			return cachedResult;
		...
		try{
			...
			CacheProvider.StoreInCache(spParameters, results);
	...
	/// </param>
	/// <returns>
	///     Returns a task representing the asynchronous operation. The task result is an <see cref="ImmutableArray" /> of
	...
	public static async Task<ImmutableArray<TResultSet>> QueryAsImmutableArrayAsync<TResultSet>(
		...
		where TResultSet : class, ISpMapper<TResultSet>
	{
		if (CacheProvider.TryRetrieveFromCache(spParameters, out ImmutableArray<TResultSet>? cachedResult) &&
		    cachedResult != null)
