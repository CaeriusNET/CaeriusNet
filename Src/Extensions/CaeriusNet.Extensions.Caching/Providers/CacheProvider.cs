...
namespace CaeriusNet.Extensions.Caching.Providers;
...
public static class CacheProvider
{
	...
	static internal bool TryRetrieveFromCache<T>(StoredProcedureParameters spParameters, out T? result)
	{
		...
		return spParameters.CacheType switch
		{
			...
			CacheType.Frozen => FrozenCacheManager.TryGet(spParameters.CacheKey, out result),
			CacheType.Redis => RedisCacheManager.IsInitialized() && RedisCacheManager.TryGet(spParameters.CacheKey, out result),
			_ => false
		};
	}
